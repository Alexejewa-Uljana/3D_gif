...
def log_scene(data):
    ...
    for ag in data["agents"]:
        ...
        cols = [oc.AXIS_COLORS["x"], oc.AXIS_COLORS["y"], oc.AXIS_COLORS["z"]]
        for i, row in enumerate(traj):
            t, x, y, z, roll, pitch, yaw = row
            set_time(t)
            pos = np.array([x * cs, y * cs, z * cs + r])
            R = oc.rot_matrix(roll, pitch, yaw)
            rr.log(f"world/agents/{name}", rr.Transform3D(translation=pos))
            rr.log(f"world/trails/{name}",
                   rr.LineStrips3D([traj[: i + 1, 1:4] * cs], colors=[color], radii=0.08))

            vecs = (R * oc.AXIS_LEN).T
            labels = [f"{np.degrees(roll):+.0f}°", f"{np.degrees(pitch):+.0f}°", f"{np.degrees(yaw):+.0f}°"]
            rr.log(f"world/angles/{name}",
                   rr.Arrows3D(origins=[pos] * 3, vectors=vecs, colors=cols,
                               labels=labels, show_labels=True, radii=0.06))


def main():
    ap = argparse.ArgumentParser(description="C: направления с подписями-значениями")
    ap.add_argument("json", nargs="?")
    ap.add_argument("--save", metavar="FILE.rrd")
    a = ap.parse_args()
    data = json.load(open(a.json, encoding="utf-8")) if a.json else oc.demo_data_6d()
    rr.init("mapf_orient_c", spawn=not a.save)
    if a.save:
        rr.save(a.save)
